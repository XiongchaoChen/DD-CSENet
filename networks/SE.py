...
class SpatialSELayer2D(nn.Module):
    """
    ...
    def __init__(self, num_channels, norm = 'None'):
        ...
        """
        super(SpatialSELayer2D, self).__init__()
    ...
    def forward(self, input_tensor, weights=None):
        ...
        :param input_tensor: X, shape = (batch_size, num_channels, D, H)
        :return: output_tensor
        """
        # channel squeeze
        batch_size, channel, D, H = input_tensor.size()
        ...
        # spatial excitation
        output_tensor = torch.mul(input_tensor, squeeze_tensor.view(batch_size, 1, D, H))

        return output_tensor


class ChannelSpatialSELayer2D(nn.Module):
    """
    ...
    def __init__(self, num_channels, reduction_ratio=2, norm='None'):
        ...
        :param reduction_ratio: By how much should the num_channels should be reduced
        """
        super(ChannelSpatialSELayer2D, self).__init__()
        self.cSE = ChannelSELayer2D(num_channels, reduction_ratio, norm=norm)
        self.sSE = SpatialSELayer2D(num_channels, norm=norm)
        self.norm = norm
